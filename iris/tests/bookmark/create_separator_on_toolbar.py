...
class Test(BaseTest):
    ...
    def run(self):
        ...
        new_separator_option_available = exists(new_separator_option_pattern, DEFAULT_SHORT_FIREFOX_TIMEOUT)
        ...
        try:
            context_menu_closed = wait_vanish(new_separator_option_pattern, DEFAULT_TINY_FIREFOX_TIMEOUT)
            assert_true(self, context_menu_closed, 'Context menu successfully closed after adding the separator')
        except FindError:
            raise FindError('Context menu did not close after adding the separator for a bookmark')

        separator_added = exists(bookmark_separator_pattern, DEFAULT_SHORT_FIREFOX_TIMEOUT)
